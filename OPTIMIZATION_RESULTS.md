...
**測試方法**: 發送超過限制的請求，檢查是否返回 429

**測試結果**:
- ✅ 一般請求限制: 60 req/min（已配置）
- ✅ 登入頁面限制: 5 req/min（已配置）
- ✅ REST API 限制: 30 req/min（已配置）
- ✅ XML-RPC 限制: 10 req/min（已配置）
- ✅ 連接數限制: 10 並發/IP（已配置）
...
**測試結果**:
- MySQL: CPU 0.67%, 記憶體 395.2 MiB
- WordPress: CPU 0.46%, 記憶體 43.23 MiB
- Nginx: CPU 0.00%, 記憶體 8.293 MiB
